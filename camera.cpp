...
third_person_camera::third_person_camera(float fov, float z_near, float z_far, float aspect, const glm::vec3 &shift)
        : perspective_camera(fov, z_near, z_far, aspect), shift_(shift) {

}

glm::mat4 third_person_camera::get_vp() {
    return perspective_camera::get_vp(target_ + shift_, target_);
}
